...
| **대상** | [hopenvision](https://github.com/bluevlad/hopenvision) (Java/Spring Boot + React) |
...
| **Tier 2** | batch (fix) | 하루 3회 (09/14/20시) | GitHub Issue 스캔 → 코드 수정 → PR 생성 |
...
│   ├── verification-reporter.ts    # 검증 요청 (Phase 4)
│   ├── docker-monitor.ts           # Docker 서비스 모니터링 (신규)
│   ├── issue-correlator.ts         # 이슈 상관관계 분석 (신규)
│   └── docker-deployer.ts          # Docker 배포 관리 (신규)
├── configs/
│   ├── projects.json               # 대상 프로젝트 + Docker 서비스 설정
│   ├── projects.local.json.example # 로컬 경로 오버라이드 예시
│   ├── approval-policy.json        # 승인 정책
│   └── schedule.json               # 3-Tier 스케줄 설정 (신규)
├── scripts/
...
- [Phase Plan](docs/PHASE_PLAN.md) - 개발 로드맵
- [Architecture](docs/ARCHITECTURE.md) - 시스템 아키텍처
- [Batch Deployment](docs/BATCH_DEPLOYMENT.md) - 운영 배포 가이드
